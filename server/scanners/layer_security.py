...
def scan_prod_readiness(repo_name: str, repo_path: str) -> list[dict]:
    ...
    has_compose = any(os.path.isfile(os.path.join(repo_path, f)) for f in
                      ["docker-compose.yml", "docker-compose.yaml"])
    if not has_dockerfile and not has_compose:
        # Check if it's a server project (has main.py, Cargo.toml, etc.)
        has_server = any(os.path.isfile(os.path.join(repo_path, f)) for f in
                         ["server/main.py", "main.py", "Cargo.toml", "pyproject.toml"])
        if has_server:
            findings.append({
    ...
    has_fastapi_app = any(
        os.path.isfile(os.path.join(d, "main.py")) and "FastAPI" in open(os.path.join(d, "main.py")).read()
        for d in server_dirs if os.path.isfile(os.path.join(d, "main.py"))
    ) if False else False
    ...
    if not has_healthcheck:
        has_server_code = any(
            os.path.isfile(os.path.join(repo_path, f)) for f in
            ["server/main.py", "main.py", "app.py", "server/app.py"]
        )
        if has_server_code:
            findings.append({
